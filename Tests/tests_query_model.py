...
def user_data(model_data:List[Dict[str, Any]], index:QtCore.QModelIndex, role:int)->Any:
    """Пользовательская функция для отображения данных в таблице"""
    if not index.isValid() or index.row() >= len(model_data) or index.column() >= len(headers):
        return None
    if role == QtCore.Qt.DisplayRole:
        if index.column() == headers.index('Исходящий'):
            ...
            res = (model_data[index.row()]['employee_last_name'] + ' '
...
def use_PyQt5():
    ...
    # Подзапрос
    subquery = (Content.select(Content.rating_id,
                               fn.COUNT(Content.id).alias("count"))
                .group_by(Content.rating_id).alias("ratings_count"))
    # Создание запроса
