...
if __name__ == "__main__":
    ...
    if DBManager().connected and not TestTable.table_exists():
        ...
        TestTable.create(
            char_field="5-я запись", datetime_field=datetime.datetime.now()
        )
        TestTable.create(char_field="6-я запись", date_field=datetime.date.today())
        TestTable.create(
            char_field="7-я запись", time_field=datetime.datetime.now().time()
        )
        TestTable.create(char_field="8-я запись", foreign_key_field=Person.get_by_id(1))
        TestTable.create(
            char_field="10-я запись", int_enum_field=SubjectiveRatingEnum.Excellent
        )
